    ...
    if("save".equals(flag) && request.getMethod().equals("POST")){
        ...
        if("true".equals(sftj)){
            ...
            if (!StringUtil.isBlankOrEmpty(connectId)) {
                ...
                if("生产".equals(nextRole)){
                    selUserId = bean.getScclr();
                }else if("技术".equals(nextRole)){
                    selUserId = bean.getJsclr();
                }else if("计划".equals(nextRole)){
                    selUserId = bean.getJhclr();
                }else if("能源办".equals(nextRole)){
                    selUserId = bean.getNybclr();
                }else if("机动科".equals(nextRole)){
                    selUserId = bean.getJdkclr();
                }else if("安全科".equals(nextRole)){
                    selUserId = bean.getAqkclr();
                }else if("工农科".equals(nextRole)){
                    selUserId = bean.getGnkclr();
                }else if("综合队".equals(nextRole)){
                    selUserId = bean.getZhdclr();
                }else if("外修队".equals(nextRole)){
                    selUserId = bean.getWxdclr();
                }else if("供应".equals(nextRole)){
                    selUserId = bean.getGyclr();
                }else if("经管".equals(nextRole)){
                    selUserId = bean.getJyclr();
                }else {
                    selUserId = "";
                }
...
<html>
	...
	<body onload="_resizeNoPage();LoadSignData();">
		<form action="sp.jsp" name="form1" method="post">
