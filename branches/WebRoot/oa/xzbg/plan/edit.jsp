...
<%
    ...
    map.put("pkid",pkid);
    map.put("fkid",pkid);
    List planlist = officePlanDAO.queryForList(map);
	...
	if (request.getMethod().equals("POST")) {
        ...
        String dxtx = StringUtil.parseNull(request.getParameter("dxtx"),"");
         ...
         officePlanProcess.setPkid(pkid);
...
//             Status status = workflow.startWorkflow("5a06a28b-133f-44d7-99da-fc5f209fbc19",cUser.getUserId());
        ...
        String PLANPKIDS[] = request.getParameterValues("PLANPKID");
        ...
        for(int j = 0;j<XMS.length;j++){
	        ...
	        mapp.put("pkid",PLANPKIDS[j]);
	        officePlan.setPkid(PLANPKIDS[j]);
	        if(officePlanDAO.queryForBean(mapp)==null){
	        	officePlanDAO.addOfficePlan(officePlan);
	        }else{
	        	officePlanDAO.modOfficePlan(officePlan);
	        }
        }
        }
%>
...
<html>
	...
	<body onload="_resizeNoPage();">
		<form action="edit.jsp" name="frmRd" method="post">
			...
			<table width="100%" height="25" border="0" cellpadding="0"
				...
				<tr>
							...
							<table width="100%" border="0" align="center" cellpadding="0"
								...
								<tr>
									...
									<td class="head_right" style="text-align: left">
                                        <input type="text" name="BT" value="<%=officePlanProcess.getBt()%>"  style="width:500px"/>
                                        <input type="checkbox" name="checked" id="checked" value="1" <%if(!"0".equals(officePlanProcess.getDxtx())){ %>checked<%}%>>短信提醒
									</td>
								</tr>
								<tr>
									<td nowrap="nowrap" width="120" class="head_left">
					...
					<%for(int i=0;i<planlist.size();i++){ 
						OfficePlan officePlan = (OfficePlan)planlist.get(i);
					%>
						<tr>
							<td>
...
function addRow(){
		...
		_td=_newRow.insertCell(_newRow.cells.length);
		_td.innerHTML='<input name="XM" id="XM'+seq+'" style="width:100%"><input type="hidden" name="PLANPKID" id="PLANPKID'+seq+'" style="width:80%" value="<%=StringUtil.getUUID()%>">';
		_td=_newRow.insertCell(_newRow.cells.length);
		...
		_td=_newRow.insertCell(_newRow.cells.length);
		_td.innerHTML='<input name="DJ" id="DJ'+seq+'" type="text" size="6" style="width:100%" onkeyup="FloatInput1(this);turn('+seq+');" onkeydown="FloatInput1(this);">';
		_td=_newRow.insertCell(_newRow.cells.length);
