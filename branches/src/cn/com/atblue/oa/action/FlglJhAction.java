...
public class FlglJhAction extends BaseAction {
private Dao dao;
private ODao oDao;
private FlglJhDAO flglJhDAO;
...
map.put("mc", mc);
int cn = flglJhDAO.getPagedCount(map);
...
this.dataList = flglJhDAO.getPagedList(map);
return "list";
}
...
Map map = new HashMap();
map.put("jhbm", bm);
this.bean = flglJhDAO.queryForBean(map);
}
return "mod";
}
...
bean.setJhbm(StringUtil.getUUID());
flglJhDAO.addFlglJh(bean);
} else if ("save".equals(action)) {
flglJhDAO.modFlglJh(bean);
}
}
return "save";
}
...
public FlglJhDAO getFlglJhDAO() {
return flglJhDAO;
}

public void setFlglJhDAO(FlglJhDAO flglJhDAO) {
this.flglJhDAO = flglJhDAO;
}

public FlglJh getBean() {
return bean;
}

public void setBean(FlglJh bean) {
this.bean = bean;
}
