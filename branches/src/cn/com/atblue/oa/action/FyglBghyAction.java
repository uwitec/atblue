...
public class FyglBghyAction extends BaseAction {
private Dao dao;
private ODao oDao;
private FyglBghyDAO fyglBghyDAO;

private FyglBghy bean;
...
map.put("mc", mc);
int cn = fyglBghyDAO.getPagedCount(map);
...
this.dataList = fyglBghyDAO.getPagedList(map);
return "list";
}
...
Map map = new HashMap();
map.put("bghybm", bm);
this.bean = fyglBghyDAO.queryForBean(map);
}
return "mod";
}
...
bean.setBghybm(StringUtil.getUUID());
fyglBghyDAO.addFyglBghy(bean);
} else if ("save".equals(action)) {
fyglBghyDAO.modFyglBghy(bean);
}
}
return "save";
}
...
public FyglBghyDAO getFyglBghyDAO() {
return fyglBghyDAO;
}

public void setFyglBghyDAO(FyglBghyDAO fyglBghyDAO) {
this.fyglBghyDAO = fyglBghyDAO;
}

public FyglBghy getBean() {
return bean;
}

public void setBean(FyglBghy bean) {
this.bean = bean;
}
