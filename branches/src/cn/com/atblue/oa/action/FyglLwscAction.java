...
public class FyglLwscAction extends BaseAction {
private Dao dao;
private ODao oDao;
private FyglLwscDAO fyglLwscDAO;

private FyglLwsc bean;
...
map.put("mc", mc);
int cn = fyglLwscDAO.getPagedCount(map);
...
this.dataList = fyglLwscDAO.getPagedList(map);
return "list";
}
...
Map map = new HashMap();
map.put("lwscbm", bm);
this.bean = fyglLwscDAO.queryForBean(map);
}
return "mod";
}
...
bean.setLwscbm(StringUtil.getUUID());
fyglLwscDAO.addFyglLwsc(bean);
} else if ("save".equals(action)) {
fyglLwscDAO.modFyglLwsc(bean);
}
}
return "save";
}
...
public FyglLwscDAO getFyglLwscDAO() {
return fyglLwscDAO;
}

public void setFyglLwscDAO(FyglLwscDAO fyglLwscDAO) {
this.fyglLwscDAO = fyglLwscDAO;
}

public FyglLwsc getBean() {
return bean;
}

public void setBean(FyglLwsc bean) {
this.bean = bean;
}
