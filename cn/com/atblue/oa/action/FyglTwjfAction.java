...
public class FyglTwjfAction extends BaseAction {
private Dao dao;
private ODao oDao;
private FyglTwjfDAO fyglTwjfDAO;

private FyglTwjf bean;
...
map.put("mc", mc);
int cn = fyglTwjfDAO.getPagedCount(map);
...
this.dataList = fyglTwjfDAO.getPagedList(map);
return "list";
}
...
Map map = new HashMap();
map.put("twjfbm", bm);
this.bean = fyglTwjfDAO.queryForBean(map);
}
return "mod";
}
...
bean.setTwjfbm(StringUtil.getUUID());
fyglTwjfDAO.addFyglTwjf(bean);
} else if ("save".equals(action)) {
fyglTwjfDAO.modFyglTwjf(bean);
}
}
return "save";
}
...
public FyglTwjfDAO getFyglTwjfDAO() {
return fyglTwjfDAO;
}

public void setFyglTwjfDAO(FyglTwjfDAO fyglTwjfDAO) {
this.fyglTwjfDAO = fyglTwjfDAO;
}

public FyglTwjf getBean() {
return bean;
}

public void setBean(FyglTwjf bean) {
this.bean = bean;
}
