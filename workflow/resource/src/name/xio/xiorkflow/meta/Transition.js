...
Transition.prototype._updatePoints = function () {
    ...
    var total = 0;
    try{
    	total = fromMetaNodeModel.tos.length;
    }catch(err){}
    ...
    if ((!fromOffset) || (!toOffset)) {
    	var offset = TransitionCompass.getOffset(fromMetaNodeModel, toMetaNodeModel, total*5);
	...
	//alert(fromOffset[1]);
    //chivas
    var from = TransitionCompass.convertOffsetToPoint(fromMetaNodeModel, fromOffset, total*5);
    var to = TransitionCompass.convertOffsetToPoint(toMetaNodeModel, toOffset, total*5);
